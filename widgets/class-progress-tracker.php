...
class SpiralEngine_Widget_Progress_Tracker extends SpiralEngine_Widget {
    ...
    public function get_schema() {
        // Schema should describe widget capabilities, not user-specific permissions
        // User-specific checks should happen at runtime, not during initialization
        return array(
            'fields' => $this->fields,
            'capabilities' => array(
                ...
                'custom_metrics' => true,
                'goal_integration' => true,  // Widget supports this feature
                'achievement_badges' => true, // Widget supports this feature
                'ai_insights' => true        // Widget supports this feature
            ),
            'analytics' => array(
            ...
            'tier_features' => $this->tier_features  // Include tier requirements for reference
        );
    }
    ...
    public function render_custom_metrics($field, $value = null) {
        // Get user tier at runtime when actually rendering
        $tier = 'free'; // Default tier
        
        // Check if we can get the actual user tier
        if (function_exists('get_current_user_id') && get_current_user_id()) {
            // Try to get tier through the parent class method if available
    ...
    public function render_analytics($user_id = null) {
        ...
        $has_achievement_badges = !empty($this->tier_features[$tier]['achievement_badges']);
        if ($has_achievement_badges && !empty($achievements)): 
        ?>
        ...
        <?php 
        // Check AI insights availability at runtime
        $has_ai_insights = !empty($this->tier_features[$tier]['ai_insights']);
        if ($has_ai_insights): 
        ?>
